...
const checks = [
  ...
  ['ESLint', 'npm', ['run', 'lint']],
  ['Ruff', '.venv/bin/ruff', ['check', 'engine', 'tests/engine', 'scripts/validate-content.py', 'tests/content/validator_test.py', 'scripts/calibrate-content.py', 'tests/content/calibration_test.py']],
  ...
  ['Stochastic calibration freshness', '.venv/bin/python', ['scripts/calibrate-content.py', '--check', '--engine', 'engine']],
  ['Player text: spelling and unfinished writing', 'node', ['scripts/check-release-text.mjs']],
  ['Curriculum document freshness', 'node', ['scripts/document-curriculum.mjs', '--check']],
  ['Models', 'node', ['scripts/validate-models.mjs']],
  ['Production build', 'npm', ['run', 'build']],
  ['Capture coverage V00-V16', 'node', ['scripts/check-captures.mjs', ...(milestone ? ['--pending-ok'] : [])]],
];
const results = [];
for (const [name, command, args] of checks) {
  ...
  // A missing interpreter or script is a failure with its own row, never a silently skipped check.
  const exit = result.error ? `error: ${result.error.code ?? result.error.message}` : result.status;
  results.push({
    check: name,
...
if (!milestone) {
  ...
  results.push({ check: `Release campaign: ${count}/77 puzzles`, result: count === 77 ? 'PASS' : 'FAIL', exit: count === 77 ? 0 : 1, seconds: '0' });
}
console.table(results);
const failures = results.filter(result => result.result === 'FAIL');
if (failures.length) {
  console.log(`\n${failures.length} failing check(s):`);
  for (const failure of failures) console.log(`  FAIL  ${failure.check}  (exit ${failure.exit})`);
}
